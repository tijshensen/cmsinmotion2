		...
		function edit() {
			...
			foreach($this->templatePage_blocks[1] as $pageBlockKey => &$pageBlockValue) {
				...
				foreach($singleLineElements[1] as $singleLineElement) {
					...
					// New input
					$returnEditor .= "<div class='input-group'>";
					$returnEditor .= "	<label>".ucfirst($XMLparser->Attributes()->name)."</label><br/>";

					$returnEditor .= "	<input onkeyup='startEditTimer(); changeRealtime(\"".sha1($XMLparser->Attributes()->name.$singleLineElement.$this->blockID)."_".$pageBlockKey."\", $(this).val());' type='text' value='".strip_tag($XMLparser->asXML(), "singleline")."' name='".sha1($XMLparser->Attributes()->name.$singleLineElement.$this->blockID)."' class='form-control' />";
					$returnEditor .= "	<span class='input-group-btn'>";
       			 	$returnEditor .= "		<input type='button' id='textlink_".$blockName."' class='btn btn-flat' value='Add link' onclick='addTextLink(\"".$blockName."\", \"".$linkValue."\", \"".$titleValue."\", \"".$targetValue."\");'>";
					$returnEditor .= "	</span>";
    				$returnEditor .= "</div>";
				...
				foreach($multiLineElements[1] as $multiLineElement) {
					...
					if ($this->newContentArray[sha1($XMLparser->Attributes()->name.$multiLineElement.$this->blockID)] != "") {
						...
						$contentShow = strip_tag($XMLparser->asXML(), "multiline");
					}
					// New textarea
					$returnEditor .= "<label>".ucfirst($XMLparser->Attributes()->name)."</label>";
				...
				foreach($fileElements[1] as $fileElement) {
					...
					// New input
					$returnEditor .= "<label>File description ".ucfirst($XMLparser->Attributes()->name)."</label>";
					...
					$returnEditor .= "<input type='button' id='file_".$blockName."' class='btn btn-primary' value='Upload file ".$XMLparser->Attributes()->name."'>";
...
";
			...
			foreach($this->templatePage_blocks[1] as $pageBlockKey => &$pageBlockValue) {
				...
				foreach($singleLineElements[1] as $singleLineElement) {
					...
					if ($this->newContentArray[sha1($XMLparser->Attributes()->name.$singleLineElement.$this->blockID)."_link"] != "") {
						...
						// New input
						$pageBlockValue = str_replace($singleLineElement, "<span id='".sha1($XMLparser->Attributes()->name.$singleLineElement.$this->blockID)."_".$pageBlockKey."'>".strip_tag($XMLparser->asXML(), "singleline")."</span>", $pageBlockValue);
		...
		function editCSS($pageBlock, $templateBlockID) {
			...
			foreach($this->templatePage_blocks[1] as $pageBlockKey => &$pageBlockValue) {
				...
				foreach($returnConstruct as $constructor) {
					...
					$returnEditor .= "<label>".($constructor)."</label>";
					$returnEditor .= "<textarea onkeyup='startEditCSSTimer(); changeCSSRealtime(\"$constructor\", $(this).val(), \"editBlock".$pageBlock."\");' name='$constructor' class='form-control'>".$this->cssContent[$constructor]."</textarea>";
		...
		function view() {
            ...
            writeRenderLog("Starting templating");
			...
			foreach($this->templatePage_blocks[1] as $pageBlockKey => &$pageBlockValue) {
				...
				// Replace all singleline elements with inputs
				preg_match_all('/(<singleline.*?>.*?<\/singleline>)/ism', $pageBlockValue, $singleLineElements);
                writeRenderLog("Matching singlelines");
				...
				foreach($singleLineElements[1] as $singleLineElement) {
                    ...
                    writeRenderLog("Matched: ".$singleLineElement." and replaced with ".$XMLparser->{0});	
					
				}	
				
				// Replace all multiline elements with textareas
				preg_match_all('/(<multiline.*?>.*?<\/multiline>)/ism', $pageBlockValue, $multiLineElements);
				writeRenderLog("Matching multilines");
				...
				foreach($multiLineElements[1] as $multiLineElement) {
                    ...
                    writeRenderLog("Matched: ".$multiLineElement." and replaced with <div>".$contentShow."</div>");					   

				}		
				
				// Replace all multiline elements with textareas
				preg_match_all('/(<img editable="true".*?\/>)/ism', $pageBlockValue, $pictureElements);
				writeRenderLog("Matching images");
